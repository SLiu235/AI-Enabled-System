...
class Reranker:
    ...
    def rerank(self, query, context, full_chuncked=None, distance_metric="cosine", k=None):
        """
        ...
        """
        self.full_corpus = full_chuncked
        if self.full_corpus:
            self.vectorizer = TfidfVectorizer(stop_words='english')
            self.vectorizer.fit(self.full_corpus)
        ...
        if self.type == "cross_encoder":
            ...
            return self.sequential_rerank(query, context, k=k)
        elif self.type == "tfidf_corpus":
            return self.tfidf_corpus_rerank(query, context, k=k)
    ...
    def sequential_rerank(self, query, context, k=None):
        """
        Applies TF-IDF, then cross-encoder reranking.

        :param query: A string containing the query.
        :param context: A list of strings, each representing a document.
        :param k: The number of top documents to retain for cross-encoder reranking.
        :return: A list of ranked documents, their indices, and relevance scores.
        """
        tfidf_documents, tfidf_indices, _ = self.tfidf_rerank(query, context)

        top_contexts = tfidf_documents[:k] if k else tfidf_documents
        return self.cross_encoder_rerank(query, top_contexts)

    def tfidf_corpus_rerank(self, query, context, k=None):
        """
        Applies TF-IDF reranking using the full corpus for term statistics.

        :param query: A string containing the query.
        :param context: A list of strings, each representing a document to be reranked.
        :param k: The number of top documents to return.
        :return: A list of top k ranked documents, their indices, and similarity scores.
        """
        if not self.full_corpus:
            raise ValueError("Full corpus must be provided during TF-IDF corpus reranking.")

        # Transform the query and context documents using the pre-fitted TF-IDF vectorizer
        query_vec = self.vectorizer.transform([query])
        context_vecs = self.vectorizer.transform(context)

        similarities = (query_vec @ context_vecs.T).toarray()[0]
        sorted_indices = np.argsort(similarities)[::-1]  

        top_indices = sorted_indices[:k] if k else sorted_indices
        ranked_documents = [context[i] for i in top_indices]
        scores = [similarities[i] for i in top_indices]

        return ranked_documents, top_indices.tolist(), scores
